        ...
        if(ecatFile.readMainHeader(pMainHeader))
        {
          ...
          if(pECAT7MainHeader)
          {
            switch(ecat7MainHeaderMap.value(g_sMainHeaderEntry))
            {
              ...
              {
                bool bSuccess = false;
                float fDosage = g_sValue.toFloat(&bSuccess);
                
                if(bSuccess)
                  pECAT7MainHeader->setDosage(fDosage);
                else
                {
                  cout << "ERROR: invalid dosage specified" << endl;
                  bResult = false;
                }
              }
              break;
...
void showHelp(int&, char** argv)
{
  ...
  cout << "  -s                     : set header entry." << endl;
  cout << "  -c                     : copy information from source file to destination." << endl;
  cout << "  -j                     : join multiple files to single one." << endl;
  cout << "  -d                     : delete information from file." << endl;
  cout << "  -n                     : create new items in file copied from source file." << endl;
  cout << "  -mhe <entry>           : specifies an entry in mainheader." << endl;
  cout << "                         : (use \"-showMainHeaderEntries\")" << endl;
  cout << "  -she <entry>           : specifies an entry in subheader" << endl;
  cout << "                         : (use \"-showSubHeaderEntries\")" << endl;
  cout << "  -m <matrixID>          : specific matrix in file via <frame,plane,gate,data,bed>." << endl;
  cout << "  -va <value>            : sets specific value." << endl;
  cout << "  -po <position>         : index position in header entry [0,1,...,N]." << endl;
  cout << "  -mh                    : flag which enables copying of mainheader." << endl;
  ...
  cout << "Examples:" << endl;
  cout << "  Get header information:" << endl;
  cout << "   ecatutil -g -mhe \"entry\" -po \"pos\" <ecatfile>" << endl;
  cout << "   ecatutil -g -she \"entry\" -m \"f,p,g,d,b\" -po \"pos\" <ecatfile>" << endl;
  cout << endl;
  cout << "  Set header information:" << endl;
  cout << "   ecatutil -s -mhe \"entry\" -va \"value\" -po \"pos\" <ecatfile>" << endl;
  cout << "   ecatutil -s -she \"entry\" -m \"f,p,g,d,b\" -va \"value\" -po \"pos\" <ecatfile>" << endl;
  cout << endl;
  cout << "  Copy information (matrix, header, etc.) from source to destination:" << endl;
  cout << "   ecatutil -c -mh -sf \"ecatfile\" <ecatfile>" << endl;
  cout << "   ecatutil -c -i -m \"f,p,g,d,b\" -sf \"ecatfile\" -dm \"f,p,g,d,b\" <ecatfile>" << endl;
  cout << endl;
  cout << "  Join multiple files to single one:" << endl;
  cout << "   ecatutil -j -sf \"file1,file2,...,fileN\" -as {\"frames\"|\"gates\"|\"beds\"} <ecatfile>" << endl;
  cout << endl;
  cout << "  Delete information/matrix from file:" << endl;
  cout << "   ecatutil -d -i -m \"f,p,g,d,b\" <ecatfile>" << endl;
  cout << endl;
  cout << "  Create new entry/matrix in file:" << endl;
  cout << "   ecatutil -n -sf -m \"f,p,g,d,b\" <ecatfile>" << endl;
}
