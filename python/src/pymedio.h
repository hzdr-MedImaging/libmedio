...
class MedIOImage
{
  public:
        ...
        m_data(nullptr)
    {
        ...
        if(eFile->readMainHeader(ecatMainHeader) == true)
        {
          if(ecatMainHeader->mainHeaderType() == CECATMainHeader::ECAT7MainHeader)
          {
            ...
            else
            {
              ...
              std::cerr << "ERROR: can handle only a single dynamic index (one of frame/gate/bed)" << std::endl;
            }
            ...
            else
            {
              ...
              for(it = volumes.begin(); it < volumes.end(); ++it)
              {
                ...
                {
                  std::cerr << "ERROR: index in volumes is exceeding boundaries "
                       << "there are " << num_volumes << " volumes in this file." << std::endl;
                }
              }
            }
                ...
                if(ecatSubHeader->subHeaderType() == CECATSubHeader::ECAT7_Image)
                {
                  ...
                  if(m_xdim != 0)
                  {
                    if(m_xdim != e7SubHeaderImage->x_Dimension())
                      std::cerr << "ERROR: unequal x dimensions in dataset" << std::endl;
                  }
                  else
                  ...
                  if(m_ydim != 0)
                  {
                    if(m_ydim != e7SubHeaderImage->y_Dimension())
                      std::cerr << "ERROR: unequal y dimensions in dataset" << std::endl;
                  }
                  else
                  ...
                  if(m_zdim != 0)
                  {
                    if(m_zdim != e7SubHeaderImage->z_Dimension())
                      std::cerr << "ERROR: unequal z dimensions in dataset" << std::endl;
                  }
                  else
                  ...
                  else
                  {
                    ...
                    for(it = rows.begin(); it < rows.end(); ++it)
                    {
                      if( (*it < 1) || (*it > m_ydim) )
                      {
                        std::cerr << "ERROR: index in rows vector is out of bounds "
                             << "there are " << m_ydim << " rows in this file." << std::endl;
                  ...
                  else
                  {
                    ...
                    for(it = cols.begin(); it < cols.end(); ++it)
                    {
                      if( (*it < 1) || (*it > m_xdim) )
                      {
                        std::cerr << "ERROR: index in cols vector is out of bounds "
                             << "there are " << m_xdim << " cols in this file." << std::endl;
                        exceeding_boundaries = true;
                      }
                    }
                  }
                  ...
                  else
                  {
                    ...
                    for(it = planes.begin(); it < planes.end(); ++it)
                    {
                      if( (*it < 1) || (*it > m_zdim))
                      {
                        std::cerr << "ERROR: index in planes std::vector is out of bounds "
                             << "there are " << m_zdim << " planes in this file." << std::endl;
                        exceeding_boundaries = true;
                      }
                    }
                  }
      ...
      //std::cout << m_numdim << " " << m_xdim << " " << m_ydim << " " << m_zdim << std::endl;
    }
