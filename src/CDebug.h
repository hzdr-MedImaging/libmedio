...
class CDebug
{
	...
	protected:
		CDebug(Level dLevel=All);
		~CDebug();
	
	private:
		static CDebug*											m_pSingletonInstance;	//!< the singleton instance
		std::map<pthread_t, long>						m_IdentLevel;					//!< different ident levels for different threads
