...
bool CECAT6MainHeader::save(CECATFile* pFile)
{
	// open the ECAT6 file to write out our main header
	// to the ecat file.
	if(pFile->open(IO_WriteOnly))
	{
		...
		// to the current one
		strncpy(m_Data.Original_File_Name, QFileInfo(pFile->name()).fileName().ascii(), 20);
