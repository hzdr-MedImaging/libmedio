...
void CECAT7MainHeader::clear()
{
	...
	strcpy(m_Data.Study_Description, (QString("Created with libmedio v") + PACKAGE_VERSION).ascii());
...
bool CECAT7MainHeader::convertFrom(const CMedIOHeader* pHead1, const CMedIOHeader* pHead2) 
{
	...
	switch(pHead1->headerFormat())
	{
		...
		case CMedIOHeader::ConcordeMicroPetMainHeader:
		{
			...
			setData_Units(concorde2ECAT7dataUnits(head->calibrationUnits()).ascii());
