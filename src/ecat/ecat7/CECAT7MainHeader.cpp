...
bool CECAT7MainHeader::convertFrom(const CMedIOHeader* mainHeader, const CMedIOHeader* subHeader)
{
  ENTER();
  bool bResult = false;
  ...
  switch(mainHeader->headerFormat())
  {
    case CMedIOHeader::ECATMainHeader:
    {
      ...
      switch(eMainHeader->mainHeaderType())
      {
        ...
        case CECATMainHeader::ECAT7MainHeader:
        {
          ...
          // for us.
          *this = *static_cast<const CECAT7MainHeader*>(mainHeader);
    ...
    case CMedIOHeader::PhilipsMainHeader:
    {
      ...
      setBed_Offset(0, head->tbl_direction() == CPhilipsMainHeader::Out ? -1 : 0);
      ...
      {
        setLwr_True_Thres(350);
        setUpr_True_Thres(650);
      }
      else if(head->hw_config() & CPhilipsMainHeader::PETMR)
      {
...
{
  strncpy(m_pData->header.Magic_Number, mn, sizeof(m_pData->header.Magic_Number)-1);
}
...
{
  strncpy(m_pData->header.Serial_Number, num, sizeof(m_pData->header.Serial_Number)-1);
}
...
{
  strncpy(m_pData->header.Isotope_Name, name, sizeof(m_pData->header.Isotope_Name)-1);
}
...
{
  strncpy(m_pData->header.Radiopharmaceutical, str, sizeof(m_pData->header.Radiopharmaceutical)-1);
}
...
{
  strncpy(m_pData->header.Study_Type, type, sizeof(m_pData->header.Study_Type)-1);
}

void CECAT7MainHeader::setPatient_ID(const char* id)                                  
{
  strncpy(m_pData->header.Patient_ID, id, sizeof(m_pData->header.Patient_ID)-1);
}

void CECAT7MainHeader::setPatient_Name(const char* name)
{
  strncpy(m_pData->header.Patient_Name, name, sizeof(m_pData->header.Patient_Name)-1);
}
...
{
  strncpy(m_pData->header.Physician_Name, name, sizeof(m_pData->header.Physician_Name)-1);
}

void CECAT7MainHeader::setOperator_Name(const char* name)                              
{
  strncpy(m_pData->header.Operator_Name, name, sizeof(m_pData->header.Operator_Name)-1);
}

void CECAT7MainHeader::setStudy_Description(const char* descr)                        
{
  strncpy(m_pData->header.Study_Description, descr, sizeof(m_pData->header.Study_Description)-1);
}
...
{
  strncpy(m_pData->header.Facility_Name, name, sizeof(m_pData->header.Facility_Name)-1);
}
...
{
  strncpy(m_pData->header.User_Process_Code, code, sizeof(m_pData->header.User_Process_Code)-1);
}
...
{ 
  strncpy(m_pData->header.Data_Units, units, sizeof(m_pData->header.Data_Units)-1);
}
...
QTextStream& operator<<(QTextStream& stream, const CECAT7MainHeader& mHeader)
{
  ...
  stream << "MAGIC_NUMBER "              << mHeader.m_pData->header.Magic_Number              << Qt::endl;
  stream << "ORIGINAL_FILE_NAME "        << mHeader.m_pData->header.Original_File_Name        << Qt::endl;
  stream << "SW_VERSION "                << mHeader.m_pData->header.SW_Version                << Qt::endl;
  stream << "SYSTEM_TYPE "              << mHeader.m_pData->header.System_Type                << Qt::endl;
  stream << "FILE_TYPE "                << mHeader.m_pData->header.File_Type                  << Qt::endl;
  stream << "SERIAL_NUMBER "            << mHeader.m_pData->header.Serial_Number              << Qt::endl;
  stream << "SCAN_START_TIME "          << mHeader.m_pData->header.Scan_Start_Time            << Qt::endl;
  stream << "ISOTOPE_NAME "              << mHeader.m_pData->header.Isotope_Name              << Qt::endl;
  stream << "ISOTOPE_HALFLIFE "          << mHeader.m_pData->header.Isotope_Halflife          << Qt::endl;
  stream << "RADIOPHARMACEUTICAL "      << mHeader.m_pData->header.Radiopharmaceutical        << Qt::endl;
  stream << "GANTRY_TILT "              << mHeader.m_pData->header.Gantry_Tilt                << Qt::endl;
  stream << "ROTATION_TILT "            << mHeader.m_pData->header.Gantry_Rotation            << Qt::endl;
  stream << "BED_ELEVATION "            << mHeader.m_pData->header.Bed_Elevation              << Qt::endl;
  stream << "INTRINSIC_TILT "            << mHeader.m_pData->header.Intrinsic_Tilt            << Qt::endl;
  stream << "WOBBLE_SPEED "              << mHeader.m_pData->header.Wobble_Speed              << Qt::endl;
  stream << "TRANSM_SOURCE_TYPE "        << mHeader.m_pData->header.Transm_Source_Type        << Qt::endl;
  stream << "DISTANCE_SCANNED "          << mHeader.m_pData->header.Distance_Scanned          << Qt::endl;
  stream << "TRANSAXIAL_FOV "            << mHeader.m_pData->header.Transaxial_FOV            << Qt::endl;
  stream << "ANGULAR_COMPRESSION "      << mHeader.m_pData->header.Angular_Compression        << Qt::endl;
  stream << "COIN_SAMP_MODE "            << mHeader.m_pData->header.Coin_Samp_Mode            << Qt::endl;
  stream << "AXIAL_SAMP_MODE "          << mHeader.m_pData->header.Axial_Samp_Mode            << Qt::endl;
  stream << "ECAT_CALIBRATION_FACTOR "  << mHeader.m_pData->header.Calibration_Factor        << Qt::endl;
  stream << "CALIBRATION_UNITS "        << mHeader.m_pData->header.Calibration_Units          << Qt::endl;
  stream << "CALIBRATION_UNITS_LABEL "  << mHeader.m_pData->header.Calibration_Units_Label    << Qt::endl;
  stream << "COMPRESSION_CODE "          << mHeader.m_pData->header.Compression_Code          << Qt::endl;
  stream << "STUDY_TYPE "                << mHeader.m_pData->header.Study_Type                << Qt::endl;
  stream << "PATIENT_ID "                << mHeader.m_pData->header.Patient_ID                << Qt::endl;
  stream << "PATIENT_NAME "              << mHeader.m_pData->header.Patient_Name              << Qt::endl;
  stream << "PATIENT_SEX "              << mHeader.m_pData->header.Patient_Sex[0]            << Qt::endl;
  stream << "PATIENT_DEXTERITY "        << mHeader.m_pData->header.Patient_Dexterity[0]      << Qt::endl;
  stream << "PATIENT_AGE "              << mHeader.m_pData->header.Patient_Age                << Qt::endl;
  stream << "PATIENT_HEIGHT "            << mHeader.m_pData->header.Patient_Height            << Qt::endl;
  stream << "PATIENT_WEIGHT "            << mHeader.m_pData->header.Patient_Weight            << Qt::endl;
  stream << "PATIENT_BIRTH_DATE "        << mHeader.m_pData->header.Patient_Birth_Date        << Qt::endl;
  stream << "PHYSICIAN_NAME "            << mHeader.m_pData->header.Physician_Name            << Qt::endl;
  stream << "OPERATOR_NAME "            << mHeader.m_pData->header.Operator_Name              << Qt::endl;
  stream << "STUDY_DESCRIPTION "        << mHeader.m_pData->header.Study_Description          << Qt::endl;
  stream << "ACQUISITION_TYPE "          << mHeader.m_pData->header.Acquisition_Type          << Qt::endl;
  stream << "PATIENT_ORIENTATION "      << mHeader.m_pData->header.Patient_Orientation        << Qt::endl;
  stream << "FACILITY_NAME "            << mHeader.m_pData->header.Facility_Name              << Qt::endl;
  stream << "NUM_PLANES "                << mHeader.m_pData->header.Num_Planes                << Qt::endl;
  stream << "NUM_FRAMES "                << mHeader.m_pData->header.Num_Frames                << Qt::endl;
  stream << "NUM_GATES "                << mHeader.m_pData->header.Num_Gates                  << Qt::endl;
  stream << "NUM_BED_POS "              << mHeader.m_pData->header.Num_Bed_Pos                << Qt::endl;
  stream << "INIT_BED_POSITION "        << mHeader.m_pData->header.Init_Bed_Position          << Qt::endl;
  stream << "BED_POSITION";
  for(int i=0; i < 15; i++)
  {
  ...
  stream << Qt::endl;
  stream << "PLANE_SEPARATION "          << mHeader.m_pData->header.Plane_Separation          << Qt::endl;
  stream << "LWR_SCTR_THRES "            << mHeader.m_pData->header.Lwr_Sctr_Thres            << Qt::endl;
  stream << "LWR_TRUE_THRES "            << mHeader.m_pData->header.Lwr_True_Thres            << Qt::endl;
  stream <<  "UPR_TRUE_THRES "            << mHeader.m_pData->header.Upr_True_Thres            << Qt::endl;
  stream <<  "USER_PROCESS_CODE "        << mHeader.m_pData->header.User_Process_Code          << Qt::endl;
  stream << "ACQUISITION_MODE "          << mHeader.m_pData->header.Acquisition_Mode          << Qt::endl;
  stream << "BIN_SIZE "                  << mHeader.m_pData->header.Bin_Size                  << Qt::endl;
  stream << "BRANCHING_FRACTION "        << mHeader.m_pData->header.Branching_Fraction        << Qt::endl;
  stream << "DOSE_START_TIME "          << mHeader.m_pData->header.Dose_Start_Time            << Qt::endl;
  stream <<  "DOSAGE "                    << mHeader.m_pData->header.Dosage                    << Qt::endl;
  stream <<  "WELL_COUNTER_CORR_FACTOR "  << mHeader.m_pData->header.Well_Counter_Corr_Factor  << Qt::endl;
  stream << "DATA_UNITS "                << mHeader.m_pData->header.Data_Units                << Qt::endl;
  stream <<  "SEPTA_STATE "              << mHeader.m_pData->header.Septa_State                << Qt::endl;
  stream <<  "FILL";
  for(int i=0; i < 6; i++)
  {
...
{
  QDateTime Jan1970(QDate(1970, 1, 1), QTime(), Qt::UTC);
  setScan_Start_Time(Jan1970.secsTo(scanStartTime.toUTC()));
}

void CECAT7MainHeader::setDose_Start_Time_Qt(const QDateTime& doseStartTime)
{
  QDateTime Jan1970(QDate(1970, 1, 1), QTime(), Qt::UTC);
  setDose_Start_Time(Jan1970.secsTo(doseStartTime.toUTC()));
}
