...
void CECAT7MainHeader::clear()
{
	...
	strcpy(m_pData->header.Study_Description, (QString("Created with libmedio v") + PACKAGE_VERSION).ascii());
...
bool CECAT7MainHeader::load(void)
{
  ...
  stream.readRawBytes(&m_pData->header.Magic_Number[0], 14);         //   0: Magic_Number
	stream.readRawBytes(&m_pData->header.Original_File_Name[0], 32);   //  14: Original_File_Name
	...
	stream.readRawBytes(&m_pData->header.Serial_Number[0], 10);        //  52: Original_File_Name
	stream >> m_pData->header.Scan_Start_Time;                         //  62: Scan_Start_Time
	stream.readRawBytes(&m_pData->header.Isotope_Name[0], 8);          //  66: Isotope_Name
	stream >> m_pData->header.Isotope_Halflife;                        //  74: Isotope_Halflife
	stream.readRawBytes(&m_pData->header.Radiopharmaceutical[0], 32);	//  78: Radiopharmaceutical
  ...
  stream.readRawBytes(&m_pData->header.Study_Type[0], 12);						// 154: Study_Type
  stream.readRawBytes(&m_pData->header.Patient_ID[0], 16);						// 166: Patient_ID
  stream.readRawBytes(&m_pData->header.Patient_Name[0], 32);					// 182: Patient_Name
  stream.readRawBytes(&m_pData->header.Patient_Sex[0], 1);						// 214: Patient_Sex
  stream.readRawBytes(&m_pData->header.Patient_Dexterity[0], 1);			// 215: Patient_Dexterity
  ...
  stream.readRawBytes(&m_pData->header.Physician_Name[0], 32);				// 232: Physician_Name
  stream.readRawBytes(&m_pData->header.Operator_Name[0], 32);				// 264: Operator_Name
  stream.readRawBytes(&m_pData->header.Study_Description[0], 32);		// 296: Study_Description
  ...
  stream.readRawBytes(&m_pData->header.Facility_Name[0], 20);				// 332: Facility_Name
  ...
  stream.readRawBytes(&m_pData->header.User_Process_Code[0], 10);		// 434: User_Process_Code
  ...
  stream.readRawBytes(&m_pData->header.Data_Units[0], 32);						// 466: Data_Units
  ...
  stream >> m_pData->header.CTI_Reserved[5];													// 510: CTI_Reserved 6
	...
	time.setTime_t(m_pData->header.Scan_Start_Time);
	D("Scan_Start_Time         : %s (%d)", 	 	    time.toString().ascii(), m_pData->header.Scan_Start_Time);
	...
	time.setTime_t(m_pData->header.Patient_Birth_Date);
	D("Patient Birthdate       : %s (%d)",				time.toString().ascii(), m_pData->header.Patient_Birth_Date);
	...
	for(int i=0; i < m_pData->header.Num_Bed_Pos; i++)
	{
	...
	time.setTime_t(m_pData->header.Dose_Start_Time);
	D("Dose start time         : %s (%d)",				time.toString().ascii(), m_pData->header.Dose_Start_Time);
	...
	while(stream.atEnd() == false)
	{
		...
		if(buf.isEmpty() == false)
		{
			...
			if(typeString == "MAGIC_NUMBER")
				strncpy(mHeader.m_pData->header.Magic_Number, dataString.ascii(), 14);
			else if(typeString == "ORIGINAL_FILE_NAME")
				strncpy(mHeader.m_pData->header.Original_File_Name, dataString.ascii(), 32);
				...
				strncpy(mHeader.m_pData->header.Serial_Number, dataString.ascii(), 10);
				...
				strncpy(mHeader.m_pData->header.Isotope_Name, dataString.ascii(), 8);
				...
				strncpy(mHeader.m_pData->header.Radiopharmaceutical, dataString.ascii(), 32);
				...
				strncpy(mHeader.m_pData->header.Study_Type, dataString.ascii(), 12);
			else if(typeString == "PATIENT_ID")
				strncpy(mHeader.m_pData->header.Patient_ID, dataString.ascii(), 16);
			else if(typeString == "PATIENT_NAME")
				strncpy(mHeader.m_pData->header.Patient_Name, dataString.ascii(), 32);
			else if(typeString == "PATIENT_SEX")
				mHeader.m_pData->header.Patient_Sex[0] = dataString.ascii()[0];
			else if(typeString == "PATIENT_DEXTERITY")
				mHeader.m_pData->header.Patient_Dexterity[0] = dataString.ascii()[0];
				...
				strncpy(mHeader.m_pData->header.Physician_Name, dataString.ascii(), 32);
			else if(typeString == "OPERATOR_NAME")
				strncpy(mHeader.m_pData->header.Operator_Name, dataString.ascii(), 32);
			else if(typeString == "STUDY_DESCRIPTION")
				strncpy(mHeader.m_pData->header.Study_Description, dataString.ascii(), 32);
				...
				strncpy(mHeader.m_pData->header.Facility_Name, dataString.ascii(), 20);
			...
			else if(typeString == "BED_POSITION")
			{
				...
				strncpy(mHeader.m_pData->header.User_Process_Code, dataString.ascii(), 10);
				...
				strncpy(mHeader.m_pData->header.Data_Units, dataString.ascii(), 32);
...
bool CECAT7MainHeader::save(void) const
{
  ...
  // as we have to care about big/little endianess, which
	// is automatically done by the QT framework
	stream.writeRawBytes(&m_pData->header.Magic_Number[0], 14);        //   0: Magic_Number
  stream.writeRawBytes(&m_pData->header.Original_File_Name[0], 32);  //  14: Original_File_Name
  stream << m_pData->header.SW_Version;                              //  46: SW_Version
  stream << m_pData->header.System_Type;                             //  48: System_Type
  stream << m_pData->header.File_Type;                               //  50: File_Type
  stream.writeRawBytes(&m_pData->header.Serial_Number[0], 10);       //  52: Original_File_Name
  stream << m_pData->header.Scan_Start_Time;                         //  62: Scan_Start_Time
  stream.writeRawBytes(&m_pData->header.Isotope_Name[0], 8);         //  66: Isotope_Name
  stream << m_pData->header.Isotope_Halflife;                        //  74: Isotope_Halflife
  stream.writeRawBytes(&m_pData->header.Radiopharmaceutical[0], 32);	//  78: Radiopharmaceutical
  stream << m_pData->header.Gantry_Tilt;															// 110: Gantry_Tilt
  stream << m_pData->header.Gantry_Rotation;													// 114: Gantry_Rotation
  stream << m_pData->header.Bed_Elevation;														// 118: Bed_Elevation
  stream << m_pData->header.Intrinsic_Tilt;													// 122: Intrinsic_Tilt
  ...
  stream << m_pData->header.Calibration_Units_Label;									// 150: Calibration_Units_Label
  stream << m_pData->header.Compression_Code;												// 152: Compression_Code
  stream.writeRawBytes(&m_pData->header.Study_Type[0], 12);					// 154: Study_Type
  stream.writeRawBytes(&m_pData->header.Patient_ID[0], 16);					// 166: Patient_ID
  stream.writeRawBytes(&m_pData->header.Patient_Name[0], 32);				// 182: Patient_Name
  stream.writeRawBytes(&m_pData->header.Patient_Sex[0], 1);					// 214: Patient_Sex
  stream.writeRawBytes(&m_pData->header.Patient_Dexterity[0], 1);		// 215: Patient_Dexterity
  stream << m_pData->header.Patient_Age;															// 216: Patient_Age
  stream << m_pData->header.Patient_Height;													// 220: Patient_Height
  stream << m_pData->header.Patient_Weight;													// 224: Patient_Weight
  stream << m_pData->header.Patient_Birth_Date;											// 228: Patient_Birth_Date
  stream.writeRawBytes(&m_pData->header.Physician_Name[0], 32);			// 232: Physician_Name
  stream.writeRawBytes(&m_pData->header.Operator_Name[0], 32);				// 264: Operator_Name
  stream.writeRawBytes(&m_pData->header.Study_Description[0], 32);		// 296: Study_Description
  stream << m_pData->header.Acquisition_Type;												// 328: Acquisition_Type
  stream << m_pData->header.Patient_Orientation;											// 330: Patient_Orientation
  stream.writeRawBytes(&m_pData->header.Facility_Name[0], 20);				// 332: Facility_Name
  stream << m_pData->header.Num_Planes;															// 352: Num_Planes
  ...
  stream << m_pData->header.Lwr_True_Thres;													// 430: Lwr_True_Thres
  stream << m_pData->header.Upr_True_Thres;													// 432: Upr_True_Thres
  stream.writeRawBytes(&m_pData->header.User_Process_Code[0], 10);		// 434: User_Process_Code
  stream << m_pData->header.Acquisition_Mode;												// 444: Acquisition_Mode
  stream << m_pData->header.Bin_Size;																// 446: Bin_Size
  stream << m_pData->header.Branching_Fraction;											// 450: Branching_Fraction
  stream << m_pData->header.Dose_Start_Time;													// 454: Dose_Start_Time
  stream << m_pData->header.Dosage;																	// 458: Dosage
  stream << m_pData->header.Well_Counter_Corr_Factor;								// 462: Well_Counter_Corr_Factor
  stream.writeRawBytes(&m_pData->header.Data_Units[0], 32);					// 466: Data_Units
  stream << m_pData->header.Septa_State;															// 498: Septa_State
  stream << m_pData->header.CTI_Reserved[0];													// 500: CTI_Reserved 1
  stream << m_pData->header.CTI_Reserved[1];													// 502: CTI_Reserved 2
  stream << m_pData->header.CTI_Reserved[2];													// 504: CTI_Reserved 3
...
bool CECAT7MainHeader::convertFrom(const CMedIOHeader* pHead1, const CMedIOHeader* pHead2) 
{
	...
	switch(pHead1->headerFormat())
	{
		...
		case CMedIOHeader::ConcordeMicroPetMainHeader:
		{
			...
			setStudy_Description(head->study());
...
{
	m_pData->header.Scan_Start_Time = static_cast<Q_INT32>(time);
}
...
{
	m_pData->header.Patient_Birth_Date = static_cast<Q_INT32>(date);
}
...
{
	m_pData->header.Dose_Start_Time = static_cast<Q_INT32>(time);
}
...
CECAT7MainHeader::Patient_Sex CECAT7MainHeaderPrivate::concorde2ECAT7Sex(const QString& genus) const
{
	...
	if(genus.startsWith("m", false))
		E7s = CECAT7MainHeader::Sex_Male;
	else if (genus.startsWith("f", false))
