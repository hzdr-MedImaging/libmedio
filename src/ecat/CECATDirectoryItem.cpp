...
class CECATDirectoryItemPrivate
{
	public:
		...
		short									bed;
		short									data;
		QIODevice::Offset			dataBlock_Start;	// also start of SubHeader
		QIODevice::Offset			dataBlock_End;		// end of raw Data area
...
Q_UINT32 CECATDirectoryItem::matrixID(void) const
{ 
...
bool CECATDirectoryItem::readMatrix(char*& matrixData, unsigned int& matrixSize)
{
	...
	if(m_pData->status == Finished)
	{
		...
		if(m_pData->file->at(m_pData->dataBlock_Start+subHeader->rawDataSize()) == false)
		{
			RETURN(false);
			return false;
		}
		...
		switch(subHeader->data_Type())
		{
			case CECATSubHeader::UnknownDataType:
			{
				...
				if(m_pData->file->readBlock(matrixData, matrixSize) > 0)
					result = true;
			}
			break;
			...
			case CECATSubHeader::VAX_Ix2:
			{
				...
				for(unsigned int read=0; read < matrixSize;)
				{
					...
					unsigned int curRead = m_pData->file->readBlock(bufArray.data(), toRead);
					if(curRead != toRead)
					{
						result = false;
						break;
																		 ...
																		 const CECATSubHeader& subHeader)
{
	...
	switch(subHeader.data_Type())
	{
		...
		case CECATSubHeader::ByteData:
		{
			if(m_pData->file->writeBlock(matrixData, matrixSize) == (Q_LONG)matrixSize)
				result = true;
		}
		break;
		...
		case CECATSubHeader::VAX_Ix2:
		{
			...
			for(unsigned int written=0; written < matrixSize;)
			{
				...
				if(m_pData->file->writeBlock(bufArray.data(), toWrite) != (Q_LONG)toWrite)
				{
					result = false;
					break;
				}
...
QDataStream& operator<<(QDataStream& stream, const CECATDirectoryItem& dItem)
{
	...
	Q_UINT32 matrixID = convertToMatrixID(dItem.m_pData->frame,
																				dItem.m_pData->plane,
																				dItem.m_pData->gate,
																				dItem.m_pData->bed,
																				dItem.m_pData->data);
	...
	Q_UINT32 dataBlock_Start = FilePos2ECATBlock(dItem.m_pData->dataBlock_Start);
	stream << dataBlock_Start;

	Q_UINT32 dataBlock_End = FilePos2ECATBlock(dItem.m_pData->dataBlock_End);
	stream << dataBlock_End;
	
	Q_INT32 matrixStatus = static_cast<Q_INT32>(dItem.m_pData->status);
	stream << matrixStatus;
