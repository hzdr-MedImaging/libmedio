...
CRECATFile::CRECATFile(CECATMainHeader::Type fileType)
  : CECATFile(fileType)
{
}
...
bool CRECATFile::writeMainHeader(Rcpp::List& mainHeader)
{
  bool result = false;

  // first we have to create a new ecat main header
  CECATMainHeader* mHeader = NULL;
  mHeader = createMainHeader(mainHeader);
  if(mHeader == NULL)
  {
  ...
  {
    result = CECATFile::writeMainHeader(*mHeader);
    delete mHeader;
  }
...
bool CRECATFile::writeSubHeader(Rcpp::List& subHeader,
                                short frame, short plane, short gate, short bed, short data)
{
  bool result = false;
  ...
  if(format() == CECATFile::ECAT7)
  {
    ...
    result = CECATFile::writeSubHeader(imageSubHeader, frame, plane, gate, bed, data);
  }
  else
    cerr << "ERROR: provided input file is not an EACAT7 file." << endl;
...
CECATMainHeader* CRECATFile::createMainHeader(Rcpp::List& rMainHeader)
{
  ...
  if(isOpen())
  {
    switch(format())
    {
      ...
      {
        pMainHeader = CECATFile::createEmptyMainHeader();
...
Rcpp::List CRECATFile::createEmptyMainHeader(void)
{
  CECATMainHeader *ecatMainHeader;
  Rcpp::List mainHeader;

  if((ecatMainHeader = CECATFile::createEmptyMainHeader()) != NULL)
  {
    if(format() == CECATFile::ECAT7)
    {
      CECAT7MainHeader *mHeader = static_cast<CECAT7MainHeader*>(ecatMainHeader);
...
Rcpp::List CRECATFile::createEmptySubHeader(void)
{
  CECATSubHeader *ecatSubHeader;
  Rcpp::List subHeader;

  if((ecatSubHeader = CECATFile::createEmptySubHeader()) != NULL)
  {
    if(format() == CECATFile::ECAT7)
    {
      CECAT7SubHeaderImage* sHeadImage = static_cast<CECAT7SubHeaderImage*>(ecatSubHeader);
